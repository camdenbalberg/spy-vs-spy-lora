...
def compute_appearance_suffix(raw_caption, spy, white_hat_off=False, black_hat_off=False):
    """Compute the default appearance suffix based on spy type and caption content.

    Returns the suffix string, or empty string if no spy name appears in caption.
    Hat-off flags swap to the hatless variant when set.
    """
    caption_lower = raw_caption.lower()
    spy_lower = (spy or "").lower()

    if spy_lower in ("white", "black"):
        if f"{spy_lower} spy" in caption_lower:
        ...
        has_white = "white spy" in caption_lower
        has_black = "black spy" in caption_lower
        if has_white and has_black:
            if white_hat_off and black_hat_off:
                return APPEARANCE_PLURAL_HATLESS
            elif not white_hat_off and not black_hat_off:
                return APPEARANCE_PLURAL
            else:
                w = APPEARANCE_SINGULAR_HATLESS if white_hat_off else APPEARANCE_SINGULAR
                b = APPEARANCE_SINGULAR_HATLESS if black_hat_off else APPEARANCE_SINGULAR
                return f"white spy {w}, black spy {b}"
        elif has_white:
            return APPEARANCE_SINGULAR_HATLESS if white_hat_off else APPEARANCE_SINGULAR
        elif has_black:
            return APPEARANCE_SINGULAR_HATLESS if black_hat_off else APPEARANCE_SINGULAR

    return ""


def build_caption_text(raw_caption, spy, appearance_suffix=None):
    """Build final caption with trigger word, appearance suffix, and style suffix.

    If appearance_suffix is provided (from manifest override), it is used as-is.
    Otherwise, the default suffix is computed from the spy type and caption content.
    An empty appearance_suffix means no appearance info is appended.
    """
    text = f"{TRIGGER_WORD}, {raw_caption}"

    if appearance_suffix is None:
        appearance_suffix = compute_appearance_suffix(raw_caption, spy)
...
def build_dataset_for_episode(episode_stem):
    ...
    for shot_info in manifest["shots"]:
        ...
        ds_txt.write_text(
            build_caption_text(raw_caption, spy, appearance_suffix=appearance_suffix),
            encoding="utf-8",
        )
        count += 1
...
def cmd_include(args):
    ...
    ds_txt = DATASET_DIR / f"{episode_stem}_{shot_prefix}.txt"
    ds_txt.write_text(build_caption_text(raw_caption, spy), encoding="utf-8")
...
def cmd_edit_caption(args):
    ...
    if not excluded:
        ds_txt = DATASET_DIR / f"{episode_stem}_{shot_prefix}.txt"
        ds_txt.write_text(build_caption_text(new_caption, spy), encoding="utf-8")
...
def cmd_summary(_args):
    ...
    print("=== SPY VS SPY DATASET SUMMARY ===")
...
def main():
    parser = argparse.ArgumentParser(
        description="Spy vs Spy LoRA training data pipeline",
