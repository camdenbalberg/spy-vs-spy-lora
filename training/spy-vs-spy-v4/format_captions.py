...
v4 changes:
- Updated conversion prompt with framing override and spy naming rules
- Spy override post-processing for hands-only frames
- Trigger word prefix uses character triggers (white_spy, black_spy)
- No style suffix
...
# Body keywords that indicate a character is present (not just disembodied parts)
BODY_KEYWORDS = {
    "face", "head", "hat", "mouth", "eyes", "eyebrows",
    "body", "standing", "sitting", "walking", "running", "crouching",
}
...
def spy_override_postprocess(tags, manifest_shot, episode_stem):
    """Post-process formatted tags to handle hands-only frames.

    If the framing is close-up/extreme close-up AND no body keywords are present,
    strip spy names and update manifest spy to None.

    Returns (updated_tags, was_overridden).
    """
    tags_lower = tags.lower()

    # Check framing
    framing_match = re.match(r"^(close-up|extreme close-up)", tags_lower)
    if not framing_match:
        return tags, False

    # Check if any body keywords are present in the caption text
    has_body = any(kw in tags_lower for kw in BODY_KEYWORDS)
    if has_body:
        return tags, False

    # This is a hands-only frame — strip spy names
    original_spy = manifest_shot.get("spy", "None")
    if original_spy.lower() == "none":
        return tags, False

    # Regex replace spy references
    cleaned = re.sub(r"white spy'?s?\b", "", tags, flags=re.IGNORECASE)
    ...
    # Update manifest
    if manifest_shot.get("spy_original") is None:
        manifest_shot["spy_original"] = original_spy
    manifest_shot["spy"] = "None"

    shot_id = f"s{manifest_shot['shot']:02d}"
    print(f"  [OVERRIDE] {episode_stem} {shot_id}: "
          f"{original_spy} -> None (hands-only frame)")
...
async def format_captions(episode_filter=None, dry_run=False):
    ...
    for i, tags in results:
        ...
        if not dry_run:
            # Apply spy override post-processing
            ...
            if manifest_shot:
                tags, was_overridden = spy_override_postprocess(
                    tags, manifest_shot, episode
                )
                if was_overridden:
                    # Write updated manifest with spy override
    ...
    # Batch-update manifests with new captions (one read/write per episode)
...
def restore_from_csv(episode_filter):
    ...
    """
    csv_path = Path(__file__).resolve().parent / "captions" / "gemini-descriptions.csv"
    ...
    for shot in manifest["shots"]:
        ...
        # Update manifest — restore spy from spy_original if it was overridden
        shot["caption"] = original_text
        if shot.get("spy_original") is not None:
            shot["spy"] = shot["spy_original"]
            shot["spy_original"] = None
        restored += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Gemini prose captions to LoRA training tags (v4)",
